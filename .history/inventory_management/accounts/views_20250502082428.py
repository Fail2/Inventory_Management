...
def grouped_product_list(request, group_by):
    if group_by == 'season':
        ...
        products_by_group = {group.id: Product.objects.filter(season=group) for group in groups}
    elif group_by == 'category':
        groups = Category.objects.all()
        products_by_group = {group.id: Product.objects.filter(category=group) for group in groups}
    else:
    ...
    return render(request, 'accounts/grouped_product_list.html', {
        ...
        'group_by': group_by,  # pass context so template knows what it's grouping by
    })
