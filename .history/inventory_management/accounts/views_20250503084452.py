...
def buyer_product_detail(request, user_type, product_id):
    ...
    if not username:
        return HttpResponse("No username found in the session.", status=400)
    
    buyer_name = request.session.get('buyer_id')
    if not buyer_id:
    ...
    try:
        ...
        return HttpResponse(f"This user '{username}' is not registered as a buyer.", status=400)
