...
def buyer_list(request):
    ...
    return render(request, 'accounts/buyer_list.html', {'buyers': buyers})
...
def add_buyer(request):
    if request.method == 'POST':
        ...
        context = {
            'buyer': buyer,
            'plain_password': password,  # Send original password, not hashed one
        }

        html_content = render_to_string('buyer_email_template.html', context)

        email_message = EmailMultiAlternatives(subject, '', from_email, to_email)
        email_message.attach_alternative(html_content, "text/html")
        email_message.send()

        messages.success(request, 'Buyer added successfully and email sent!')
