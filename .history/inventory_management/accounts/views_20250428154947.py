...
def logout_view(request):
    if request.method == "POST":
        if request.user.is_superuser:
            ...
            # For non-superusers, flush the session before logging out
            # Buyer/Supplier: Flush session manually
            request.session.flush()
            logout(request)
            response = redirect('login')

        # Set a logout message inside a cookie
        response.set_cookie('logout_message', "You have logged out successfully.", max_age=5)
