...
def product_list(request):
    products = Product.objects.select_related('category', 'supplier', 'season').all()
    return render(request, 'accounts/product_list.html', {'products': products})

#add-product
@login_required(login_url='admin_login')  # Ensure user is logged in
def add_product(request):
    print("User:", request.user)
    print("Is authenticated:", request.user.is_authenticated)
    print("Is superuser:", request.user.is_superuser)

    # Optional: restrict access to superusers (admins)
