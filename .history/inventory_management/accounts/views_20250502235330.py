...
def buyer_product_detail(request, user_type, product_id):
    ...
    if user_type != 'buyer':
        return HttpResponse("Only buyers can place orders.", status=403)
    
    buyer = Buyer.objects.filter(username=request.user.username).first()
    if not buyer:
        return HttpResponse("No buyer profile found for this user.", status=400)
    ...
    if request.method == 'POST':
        ...
        return redirect('buyer_orders')  # Adjust this to your orders page
