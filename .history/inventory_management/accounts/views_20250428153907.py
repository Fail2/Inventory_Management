    ...
    if request.method == "POST":
        if request.user.is_superuser:
            ...
            return redirect('admin_login')  # Adjust the URL name as per your setup
             ...
             # For normal users, manually store messages before flushing session
            storage = messages.get_messages(request)
            list(storage)  # This is needed to "consume" the messages before flushing
            request.session.flush()
            # Now recreate the message storage after flushing
            request._messages = storage
            messages.success(request, "You have logged out successfully.")
            logout(request)
            # Redirect normal users to the login page after logout
