...
def edit_buyer(request, buyer_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Buyer details updated successfully!")
            return redirect('buyer_list')  # Redirect to the buyer list page
    else:
        form = BuyerForm(instance=buyer)
    
    return render(request, 'accounts/edit_buyer.html', {'form': form})

# Delete Buyer View
def delete_buyer(request, buyer_id):
    if request.method == "POST":
        # Get the buyer object based on the provided ID
        buyer = get_object_or_404(Buyer, id=buyer_id)
        ...
        # Redirect to the buyer list page
        return redirect('buyer_list')
    else:
        # If the request is not POST, redirect to the buyer list page
        return redirect('buyer_list')
