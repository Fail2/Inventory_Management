...
def product_detail(request, product_id):
    ...
    if request.method == 'POST':
        ...
        buyer = request.user  # assumes logged-in user is the buyer

        if quantity < 1:
            messages.error(request, 'Quantity must be at least 1.')
        elif quantity > product.quantity:
            messages.error(request, 'Cannot order more than available stock.')
            ...
            messages.success(request, f'Order placed successfully for {quantity} item(s) of {product.name}!')
            return redirect('buyer_home')  # adjust this to your buyer home view
