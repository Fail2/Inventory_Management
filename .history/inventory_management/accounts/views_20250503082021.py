...
def buyer_product_detail(request, user_type, product_id):
    ...
    if request.method == 'POST':
        ...
        # Check if logged-in user's username exists in Buyer table
        try:
            buyer = Buyer.objects.get(username=request.user.username)
        except Buyer.DoesNotExist:
            return HttpResponse(f"This user '{request.user.username}' is not registered as a buyer.", status=400)
        ...
        return redirect('buyer_orders')  # Adjust to your correct orders page name
