...
def buyer_product_detail(request, product_id):
    product = get_object_or_404(Product, id=product_id)

    if request.method == 'POST':
        # Check if the logged-in user has an associated Buyer object
        ...
        if quantity < 1:
            messages.error(request, 'Order quantity must be at least 1.')
            ...
            return redirect('buyer_home')  # Or redirect to a success page

    return render(request, 'buyer_product_detail.html', {'product': product})
